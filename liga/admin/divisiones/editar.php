...
$errores = [];
$id_division = filter_input(INPUT_GET, 'id', FILTER_VALIDATE_INT);
$division = null;

if ($id_division > 0) {
    try {
        $stmt = $pdo->prepare("SELECT * FROM divisiones WHERE id_division = :id");
        $stmt->bindParam(':id', $id_division, PDO::PARAM_INT);
        $stmt->execute();
        $division = $stmt->fetch();

        if (!$division) {
            $_SESSION['mensaje'] = 'División no encontrada.';
        ...
        $_SESSION['mensaje'] = 'Error al obtener datos de la división: ' . $e->getMessage();
        $_SESSION['tipo_mensaje'] = 'danger';
        header('Location: index.php');
        exit();
    }
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        if (empty($errores)) {
            try {
                $stmt = $pdo->prepare("UPDATE divisiones SET nombre = :nombre, orden = :orden WHERE id_division = :id");
                $stmt->bindParam(':nombre', $nombre);
                $stmt->bindParam(':orden', $orden, PDO::PARAM_INT);
                $stmt->bindParam(':id', $id_division, PDO::PARAM_INT);

                if ($stmt->execute()) {
                    $_SESSION['mensaje'] = 'División actualizada correctamente.';
                    $_SESSION['tipo_mensaje'] = 'success';
                    header('Location: index.php');
                    exit();
                } else {
                    $errores['general'] = 'Hubo un error al actualizar la división.';
                }
            } catch (PDOException $e) {
    ...
    $_SESSION['mensaje'] = 'ID de división inválido.';
    $_SESSION['tipo_mensaje'] = 'danger';
    header('Location: index.php');
    exit();
}
...
<div class="container my-4">
        ...
        <div class="preview-panel">
            <div class="division-name" id="preview-name"><?= htmlspecialchars($nombre); ?></div>
            <div>Orden: <span class="division-order" id="preview-order"><?= htmlspecialchars($orden); ?></span></div>
        </div>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            nameInput.addEventListener('input', function() {
                previewName.textContent = this.value || 'Nombre de la División';
            });
